...
if(isset($_POST['submit'])){
    ...
    $query = "UPDATE volunteer SET fullname='[$fullname]',age='[$age]',gender='[$gender]',activity='[$activity]',reason='[$reason]',activitydate='[$date]',status='[$status]',community='[$community]' WHERE phone='[$phone]'";
    // Eksekusi query
...
<body>
    ...
    <?php
    $phone = $_GET['id'];
        $volunteer= mysqli_query($koneksi,"select * from volunteer where phone='$phone'");
    ...
    <div class="w3-container w3-padding-24">
        <form method="POST" class="w3-container w3-card-4 w3-round-xlarge">
            <p>
                ...
                <input class="w3-input" type="text" name="phone" placeholder="Nomor Hp" value="<?php echo $data['phone']; ?>" readonly>
            </p>
            <p>
                <label for="fullname">Nama lengkap :</label>
                <input class="w3-input" type="text" name="fullname" placeholder="nama lengkap" value="<?php echo $data['fullname'] ?>" required>
            </p>
            <p>
            ...
            <p>
                ...
                <input class="w3-input" type="number" min="0" name="age" placeholder="umur" value="<?php echo $data['age'] ?>" required>
            </p>
            <p>
                <label for="activity">aktivitas yang ingin di ikuti :</label>
                <input class="w3-input" type="text" name="activity" placeholder="aktivitas yang ingin di ikuti" value="<?php echo $data['activity'] ?>" required>
            </p>
            <p>
                <label for="reason">alasan :</label>
                <input class="w3-input" type="text" name="reason" placeholder="alasan" value="<?php echo $data['reason'] ?>" required>
            </p>
            <p>
                <label for="date">tanggal aktivitas :</label>
                <input class="w3-input" type="date" name="date" placeholder="tanggal aktivitas" value="<?php echo $data['activitydate'] ?>" required>
            </p>
            <p>
                <label for="status">status :</label>
                <input class="w3-check" type="checkbox" name="status" <?php if($data['status']==1){echo "checked";}?> >
                <label>setuju mengikuti kegiatan dan menjaga alam sekitar</label>
            </p>
            <p>
                <label for="community">Asal komunitas/sekolah/instansi :</label>
                <input class="w3-input" type="text" name="community" placeholder="Asal komunitas/sekolah/instansi :" value="<?php echo $data['community'] ?>" required>
            </p>
            <p>
                <button class="w3-button w3-green w3-round-large" type="submit" name="submit">update</button>
            </p>
        </form>
    </div>
    <?php
